...
CORRECTION TO ARCHITECTURE.md §9.1 (made after the first real run):
...
def check_customer_viability(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "detail": (
            ...
            "the feature plan falls back to §4.1 (see ARCHITECTURE.md §4.2)."
        ),
    }
...
def write_data_notes(
    ...
    lines = []
    lines.append("# Data Notes — Gate Findings\n")
    lines.append(
    ...
    lines.append(
        "\n**Method correction.** ARCHITECTURE.md §9.1 specifies a *depth-1* "
    ...
    if customer_result["median_rows_per_customer"] in (None, 1.0):
        lines.append(
            ...
            "the feature plan falls back to §4.1 transaction-level features only. "
